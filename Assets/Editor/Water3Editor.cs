...
{	
	...
	{		
		...
		if(m_WaterDisplacement.boolValue) {
			...
			EditorGUILayout.BeginHorizontal();
			m_Displacement = (Texture2D) EditorGUILayout.ObjectField("Height map A", m_Displacement as Object, typeof(Texture2D), allowSceneObjects : true);
			m_2ndDisplacement = (Texture2D) EditorGUILayout.ObjectField("Height map B", m_2ndDisplacement as Object, typeof(Texture2D), allowSceneObjects : true);
		...
		*/
		...
		EditorGUILayout.BeginHorizontal();
		m_NormalMap = EditorGUILayout.ObjectField("Bump", m_NormalMap as Object, typeof(Texture2D), allowSceneObjects : true) as Texture2D;
		Water3Manager.Instance().SetMaterialTexture("_BumpMap",m_NormalMap);
		m_FoamMap = EditorGUILayout.ObjectField("Foam", m_FoamMap as Object, typeof(Texture2D), allowSceneObjects : true) as Texture2D;
		Water3Manager.Instance().SetMaterialTexture("_ShoreTex",m_FoamMap);
		EditorGUILayout.EndHorizontal();	
		EditorGUILayout.BeginHorizontal();
		m_CubeFallback = EditorGUILayout.ObjectField("Static cubemap", m_CubeFallback as Object, typeof(Cubemap), allowSceneObjects : true) as Cubemap;
		Water3Manager.Instance().SetMaterialTexture("_ReflectiveColorCube",m_CubeFallback);
		m_Fallback = EditorGUILayout.ObjectField("Fallback", m_Fallback as Object, typeof(Texture2D), allowSceneObjects : true) as Texture2D;
