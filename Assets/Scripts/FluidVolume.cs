...
public class FluidVolume : MonoBehaviour
{
	...
	{
		return new Vector3(worldPoint.x, collider.bounds.max.y + waveAmplitude * (WaveFunction(worldPoint) - 1.0f), worldPoint.z);
	}
	...
	{
		return collider.bounds.size.y + waveAmplitude * (WaveFunction(worldPoint) - 1.0f);
	}
